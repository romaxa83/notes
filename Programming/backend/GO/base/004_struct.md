...
##### 🔸 Тег `omitzero`
Тег `omitzero` убирает поля со значениями по умолчанию из JSON. Раньше использовали `omitempty`. Он пропускает поля, если они пустые: строка без символов, слайс без элементов или `nil`. Но для чисел и дат проблема. Нулевое значение `int` или `time.Time` всегда попадало в JSON, даже если данных нет.
`omitzero` решает это. Поле пропускается при нулевом значении типа: `0` для `int`, `false` для `bool`, пустая строка. Для `time.Time` не нужно делать указатель. 
