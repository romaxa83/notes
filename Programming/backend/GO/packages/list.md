#go #package

##### 🔸 CLI APP
 > для реализации консольных приложений 
- https://github.com/spf13/cobra
---
##### 🔸 Пакет для ssl сертификатов
- golang.org/x/crypto/acme/autocert
---
##### 🔸 Websocket
- https://github.com/gorilla/websocket/tree/master/examples/echo
---
##### 🔸 Viper
> позволяет удобно работать с конфигурацией
- https://github.com/spf13/viper

```bash
# установка
go get -u github.com/spf13/viper
```
---
##### 🔸 GraphQL
- https://github.com/99designs/gqlgen
---
##### 🔸 Swagger
- https://github.com/go-swagger/go-swagger
- https://github.com/swaggo/swag
---
##### 🔸  Telegram Bot Api
- https://github.com/go-telegram-bot-api/telegram-bot-api
---
##### 🔸 ORM
> gorm
- github.com/go-gorm/gorm
>  ent
- github.com/ent/ent
> pop
- https://github.com/gobuffalo/pop
---
##### 🔸 UUID
- https://github.com/google/uuid
---
##### 🔸 Пакет для валидации данных
- https://github.com/go-ozzo/ozzo-validation
- https://github.com/go-playground/validator
---
##### 🔸 Logger
- https://github.com/sirupsen/logrus
- https://github.com/uber-go/zap
---
##### 🔸 Kafka
- https://github.com/segmentio/kafka-go
---
##### 🔸 Пакет для парсинга `toml` файлов
- https://github.com/BurntSushi/toml
---
##### 🔸 Пакет для роутинга
- https://github.com/gorilla/mux
---
##### 🔸 Пакет для тестирования
- https://github.com/stretchr/testify
---
##### 🔸 Надстройка над стандартным пакетом для работы с бд
- https://github.com/jmoiron/sqlx
---
##### 🔸 Пакет для миграций
- https://github.com/golang-migrate/migrate
---
##### 🔸 Пакет для шифрования
- golang.org/x/crypto
---
##### 🔸 Пакеты от gorilla для веба
- https://www.gorillatoolkit.org/
---
##### 🔸 Рекогносцировка лица
- https://github.com/Kagami/go-face
---
